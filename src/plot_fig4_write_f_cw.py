from get_data import file_to_numpy, cycle_to_index, conc_dir_to_conc, index_to_cycle
...
def main(data):
    ...
    for reporter, values in data.items():
        ...
        for icycle in range(45):
            ...
            # C = np.zeros(N)
            # i = 0
            # for conc, val in values.items():
            #     indices = slice(i*96, (i + 1)*96)
            #     C[indices] = conc
            #     F[indices] = val[icycle, :]
            #     i += 1

            # mu[icycle] = np.inner(F, C)/np.inner(C, C)
            # err = F - mu[icycle]*C
            # s[icycle] = np.sqrt(
            #     np.sum(err*err)/(N - 1) / np.inner(C, C)
            #     )
            for iwell in range(96):
                ...
                df[icycle, iwell] = np.sqrt(np.inner(R, R)/(ncs - 1))
        ...
        with open(os.path.join("out", "f_cw_%s.npy" % reporter), "wb") as file:
            np.save(file, f)
...
if __name__ == '__main__':
    ...
    data = {
        key: get_dataset(key, TE_dir) for key in ("FAM", "Probe")
    }
